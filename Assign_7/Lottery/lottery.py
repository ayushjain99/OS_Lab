...
"""
print ''
print 'Input taken from file is as follows : '
import operator
tickTotal = 0
runTotal  = 0
joblist = []
if options.jlist == '':
    for jobnum in range(0,options.jobs):
        ...
        print 'Process %d - Run_time = %d, tickets = %d' % (jobnum, runtime, tickets)
else:
    jobnum = 0
    for entry in options.jlist.split(','):
        ...
        jobnum += 1
    for job in joblist:
        print 'Process %d Run_time = %d, tickets = %d' % (job[0], job[1], job[2])
print '\n'
...
if options.solve == True:
    print 'Scheduling is as follows :\n'
    jobs  = len(joblist)
    clock = 0
    for i in range(runTotal):
        r = int(random.random() * 1000001)
        ...
        print 'Ticket generated %d - Run Process %d' % (winner, wjob)
        print 'Jobs: ',
        for (job, runtime, tickets) in joblist:
            if wjob == job:
                wstr = ''
            ...
            if runtime > 0:
                ...
                tstr = '---'
            print '%s job:%d timeleft:%d' % (wstr, job, runtime),
        print ''
        print ''
        ...
        if wrun == 0:
            print 'Process %d finished at time %d\n' % (wjob, clock)
            tickTotal -= wtix
            wtix = 0
            jobs -= 1
