...
# Implementation of Multi-level Feedback Queue(MLFQ)
import sys
...
# finds the highest nonempty queue, -1 if they are all empty
def FindQueue():
    q = hiQueue
    while q > 0:
        if len(queue[q]) > 0:
...
numJobs = len(job)
'''
print 'Number of Processes',            numJobs
print 'Number of Queues',          numQueues
...
print 'Boost time',           options.boost
print 'Input/Output Time',          options.ioTime
...
for i in range(numJobs):
    print 'Process %2d: arrivalTime %3d - CPU_Burst_needed %3d - ioFreq %3d' % (i, job[i]['startTime'],
                                                                   job[i]['runTime'],
                                                                   job[i]['ioFreq'])
print ''
...
while finishedJobs < totalJobs:
    if options.boost > 0 and currTime != 0:
        if currTime % options.boost == 0:
            print 't = %d boost priority' % (currTime)
    ...
    if currTime in ioDone:
        for (j, type) in ioDone[currTime]:
            q = job[j]['currPri']
            job[j]['doingIO'] = False
            print 't = %d %s by Process %d' % (currTime, type, j)
    ...
    print 't = %d Run Process %d at Priority %d' % (currTime, currJob, currQueue)
    ...
    if timeLeft == 0:
        print 't = %d Finished Process %d' % (currTime, currJob)
        finishedJobs += 1
    ...
    if ioFreq > 0 and (((runTime - timeLeft) % ioFreq) == 0):
        # time for an IO!
        print 't = %d IO_START by Process %d' % (currTime, currJob)
        issuedIO = True
...
print ''
print 'Calculations are as follows :'
responseSum   = 0
turnaroundSum = 0
for i in range(numJobs):
    response   = job[i]['firstRun'] - job[i]['startTime']
    turnaround = job[i]['endTime'] - job[i]['startTime']
    print 'Process %2d: arrival_time %3d - response %3d - turnaround %3d' % (i, job[i]['startTime'],
                                                                        response, turnaround)
    responseSum   += response
    turnaroundSum += turnaround

print '\nAverage %2d: waiting_time 10.6 - response %.2f - turnaround %.2f' % (i,
